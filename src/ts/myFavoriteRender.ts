const axios = require('axios');

const fetchMyFavorite = async () => {
  const res = await axios.get('http://localhost:7000/myFavorite');
  let html = '';
  
  res.data.forEach((myFavorite: any) => {
    html += `<li id="${myFavorite.id}" class="myway__item">
    <figure>
      <img
        src="${myFavorite.item[1].url}"
        alt="${myFavorite.item[1].name}"
      />
      <figcaption>${myFavorite.name}</figcaption>
    </figure>
  </li>`;

  const $mywayList = document.querySelector('.myway__list') as HTMLUListElement;
  $mywayList.innerHTML = html;
  });

  // template
  /*
  `<li class="myway__item">
            <figure>
              <img
                src="https://m.subway.co.kr/images/menu/sandwich_cl03.jpg"
                alt="subway-meatball"
              />
              <figcaption>꿀조합</figcaption>
            </figure>
          </li>`
  */
};

export default fetchMyFavorite;